...
export const setupMbtiCommand = (bot: Telegraf<Context>) => {
  bot.command('mbti', async (ctx: Context) => {
    ...
    if (!message || !('text' in message)) {
      await _sendMessage(ctx, MESSAGE_TYPES.text, 'I can only process text messages.');
      return;
    }
    ...
    switch (secondArg) {
        ...
        if (!thirdArg) {
          ...
          if (userMbti)
            await _sendMessage(
              ctx,
              MESSAGE_TYPES.text,
              `${userMbti.userName}, your MBTI is ${userMbti.mbti}.`
            );
          else
            await _sendMessage(ctx, MESSAGE_TYPES.text, `You don't have an MBTI set yet.`, {
              reply: true,
            });
        } else {
          if (thirdArg.length !== 4) {
            await _sendMessage(ctx, MESSAGE_TYPES.text, `That's not an MBTI, baka.`, {
              reply: true,
            });
            return;
          }
          ...
          await dataController.remove(
            chatId,
            TableType.MBTI,
            (entry) => entry.userId === String(ctx.from?.id)
          );

          // Add new one
          await dataController.addMbti(
            chatId,
            String(ctx.from?.id),
          ...
          await _sendMessage(
            ctx,
            MESSAGE_TYPES.text,
            `${ctx.from?.first_name}'s MBTI is now ${formatted}`,
            {
              reply: true,
            }
          );
      ...
      default:
        await _sendMessage(ctx, MESSAGE_TYPES.text, `Second argument can only be 'me' or 'help'.`, {
          reply: true,
        });
        break;
    }
